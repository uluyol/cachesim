...
concurrent() (
    ...
    __crt__mark_task_with_code() {
        ...
        local code=${2}
        local runtime=${3}
        echo "task:${task}:${code}:${runtime}" >> "${__crt__event_pipe}"
    }
    ...
    __crt__task_runner() (
        ...
        __crt__mark_task_with_code "${1}" "${code}" "$(( $(date +%s) - start_time ))"
    )
    ...
    __crt__skip_task() {
        ...
        __crt__mark_task_with_code "${1}" skip -1
    }
    ...
    __crt__handle_done_task() {
        local IFS=:
        local index_code_rt=($1)
        unset IFS
        local index=${index_code_rt[0]}
        local code=${index_code_rt[1]}
        local runtime=${index_code_rt[2]}
        __crt__runtimes[index]="${runtime}"
    ...
    __crt__runtimes=()   # start times by index
    ...
    __crt__txtred=$(tput setaf 1) # Red
    __crt__txtgrn=$(tput setaf 2) # Green
    __crt__txtylw=$(tput setaf 3) # Yellow
    __crt__txtblu=$(tput setaf 4) # Blue
    __crt__txtbld=$(tput bold)    # Bold
    __crt__txtrst=$(tput sgr0)    # Text Reset
    __crt__txtclr='\e[0K'         # Clear to end of line

    export CONCURRENT_COMPACT=${CONCURRENT_COMPACT:-0}
    export CONCURRENT_SIMPLE=${CONCURRENT_SIMPLE:-1}

    __crt__use_compact_status() {
        [[ "${CONCURRENT_COMPACT}" != "0" ]] || [[ "${CONCURRENT_SIMPLE}" == "0" && "${__crt__task_count}" -ge "$(tput lines)" ]]
    }

    __crt__use_simple_status() {
        [[ "${CONCURRENT_SIMPLE}" != "0" || $TERM == "dumb" ]]
    }
    ...
    if __crt__use_compact_status; then
    ...
    elif __crt__use_simple_status; then
        __crt__cols=$(tput cols)
        ...
        __crt__draw_status() {
            ...
            local rt=-1
            if [[ ${code} == "0" ]]; then
                rt=${__crt__runtimes[index]}
            fi
            printf "%s " "$(__crt__fmt_rt 7 ${rt})"
            __crt__draw_task "${code}"
            echo ${__crt__names[index]}
            #__crt__move_cursor_to_index "${index}"
            #__crt__draw_task "${code}"
            #__crt__move_cursor_below_tasks
            #[[ "${code}" == "running" ]] || __crt__draw_recent_verbose_task "${index}" "${code}"
        ...
        __crt__draw_task() {
            local code=${1}
            if   [[ "${code}" == "int"     ]]; then printf "${__crt__txtred}%s${__crt__txtrst} " 'SIGINT'
            elif [[ "${code}" == "skip"    ]]; then printf "${__crt__txtylw}%s${__crt__txtrst} " ' SKIP '
            elif [[ "${code}" == "running" ]]; then printf "${__crt__txtblu}%s${__crt__txtrst} " 'LAUNCH'
